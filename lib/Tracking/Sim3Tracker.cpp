...
#define callOptimized(function, arguments)                                     \
  (USESSE ? function##SSE arguments : function arguments)
#else
...
Sim3Tracker::Sim3Tracker(const ImageSize &sz)
    : settings(), _imgSize(sz), _debugImages(sz) {
  ...
  lastResidual = 0;
  iterationNumber = 0;
  lastDepthResidual = lastPhotometricResidual = lastDepthResidualUnweighted =
      lastPhotometricResidualUnweighted = lastResidualUnweighted = 0;
  pointUsage = 0;
}
...
Sim3 Sim3Tracker::trackFrameSim3(
    ...
    const Sim3 &frameToReference_initialEstimate, int startLevel,
    int finalLevel) {
  ...
  for (int lvl = startLevel; lvl >= finalLevel; lvl--) {
    ...
    if (buf_warped_size < 0.5 * MIN_GOODPERALL_PIXEL_ABSMIN *
                              (_imgSize.width >> lvl) *
                              (_imgSize.height >> lvl) ||
        buf_warped_size < 10) {
      diverged = true;
    ...
    Sim3ResidualStruct lastErr =
        callOptimized(calcSim3WeightsAndResidual, (referenceToFrame));
    if (plotSim3TrackingIterationInfo)
      callOptimized(calcSim3Buffers,
                    (reference, frame, referenceToFrame, lvl, true));
    ...
    for (int iteration = 0; iteration < settings.maxItsPerLvl[lvl];
      ...
      while (true) {
        ...
        // test.
        Sim3 new_referenceToFrame =
            Sim3::exp(inc.cast<sophusType>()) * referenceToFrame;
        // Sim3 new_referenceToFrame = referenceToFrame * Sim3::exp((inc));

        // re-evaluate residual
        callOptimized(calcSim3Buffers,
                      (reference, frame, new_referenceToFrame, lvl));
        if (buf_warped_size < 0.5 * MIN_GOODPERALL_PIXEL_ABSMIN *
                                  (_imgSize.width >> lvl) *
                                  (_imgSize.height >> lvl) ||
            buf_warped_size < 10) {
        ...
        Sim3ResidualStruct error =
            callOptimized(calcSim3WeightsAndResidual, (new_referenceToFrame));
        if (plotSim3TrackingIterationInfo)
          callOptimized(calcSim3Buffers,
                        (reference, frame, new_referenceToFrame, lvl, true));
        numCalcResidualCalls[lvl]++;
        ...
        if (error.mean < lastErr.mean) {
          ...
          // debug output
          LOGF_IF(DEBUG, Conf().print.trackingIterationInfo,
                  "(%d-%d): ACCEPTED increment of %f with lambda %.1f, "
                  "residual: %f -> %f\n",
                  lvl, iteration, sqrt(inc.dot(inc)), LM_lambda, lastErr.mean,
                  error.mean);

          LOGF_IF(DEBUG, Conf().print.trackingIterationInfo,
                  "         p=%.4f %.4f %.4f %.4f %.4f %.4f %.4f\n",
                  referenceToFrame.log()[0], referenceToFrame.log()[1],
                  referenceToFrame.log()[2], referenceToFrame.log()[3],
                  referenceToFrame.log()[4], referenceToFrame.log()[5],
                  referenceToFrame.log()[6]);

          // converged?
          if (error.mean / lastErr.mean > settings.convergenceEps[lvl]) {
            LOGF_IF(DEBUG, Conf().print.trackingIterationInfo,
                    "(%d-%d): FINISHED pyramid level (last residual reduction "
                    "too small).\n",
                    lvl, iteration);
          ...
          LOGF_IF(DEBUG, Conf().print.trackingIterationInfo,
                  "(%d-%d): REJECTED increment of %f with lambda %.1f, "
                  "(residual: %f -> %f)\n",
                  lvl, iteration, sqrt(inc.dot(inc)), LM_lambda, lastErr.mean,
                  error.mean);
  ...
  if (Conf().print.trackingIterationInfo) {
    ...
    printf("pOld = %.5f %.5f %.5f %.5f %.5f %.5f %.5f\n",
           frameToReference_initialEstimate.inverse().log()[0],
           frameToReference_initialEstimate.inverse().log()[1],
           frameToReference_initialEstimate.inverse().log()[2],
           frameToReference_initialEstimate.inverse().log()[3],
           frameToReference_initialEstimate.inverse().log()[4],
           frameToReference_initialEstimate.inverse().log()[5],
    ...
    printf("final res mean: %f meanD %f, meanP %f\n", finalResidual.mean,
           finalResidual.meanD, finalResidual.meanP);
  }

  // Make sure that there is a warp update at the final position to get the
  // correct information matrix
  if (!warp_update_up_to_date) {
    reference->makePointCloud(finalLevel);
    callOptimized(calcSim3Buffers,
                  (reference, frame, referenceToFrame, finalLevel));
    finalResidual =
        callOptimized(calcSim3WeightsAndResidual, (referenceToFrame));
    callOptimized(calcSim3LGS, (ls7));
  }
...
void Sim3Tracker::calcSim3BuffersSSE(
    const std::shared_ptr<TrackingReference> &reference,
    const std::shared_ptr<Frame> &frame, const Sim3 &referenceToFrame,
    int level, bool plotWeights) {
  calcSim3Buffers(reference, frame, referenceToFrame, level, plotWeights);
}
#endif
...
void Sim3Tracker::calcSim3Buffers(
  ...
  Eigen::Matrix3f rotMatUnscaled =
      referenceToFrame.rotationMatrix().cast<float>();
  ...
  Eigen::Matrix3f rollMat =
      shortestBackRotation.toRotationMatrix() * rotMatUnscaled;
  ...
  const Eigen::Vector3f *refPoint_max =
      reference->posData[level] + reference->numData[level];
  ...
  for (; refPoint < refPoint_max; refPoint++, refGrad++, refColVar++) {
    ...
    *(buf_warped_z + idx) = Wxp(2);
    ...
    Eigen::Vector3f resInterp =
        getInterpolatedElement43(frame_intensityAndGradients, u_new, v_new, w);
    ...
    *(buf_d + idx) = 1.0f / (*refPoint)[2];
    ...
    if (plotSim3TrackingIterationInfo) {
      ...
      setPixelInCvMat(&_debugImages.debugImageOldImageSource,
                      getGrayCvPixel((float)resInterp[2]), u_new + 0.5,
                      v_new + 0.5, (width / w));
      setPixelInCvMat(&_debugImages.debugImageOldImageWarped,
                      getGrayCvPixel((float)resInterp[2]), x, y, (width / w));
      setPixelInCvMat(&_debugImages.debugImageResiduals,
                      getGrayCvPixel(residual_p + 128), x, y, (width / w));

      if (*(buf_warped_idepthVar + idx) >= 0) {
        setPixelInCvMat(&_debugImages.debugImageDepthResiduals,
                        getGrayCvPixel(128 + 800 * *(buf_residual_d + idx)), x,
                        y, (width / w));

        if (plotWeights) {
          setPixelInCvMat(&_debugImages.debugImageWeightD,
                          getGrayCvPixel(255 * (1 / 60.0f) *
                                         sqrtf(*(buf_weight_VarD + idx))),
                          x, y, (width / w));
          setPixelInCvMat(
              &_debugImages.debugImageWeightedResD,
              getGrayCvPixel(128 + (128 / 5.0f) *
                                       sqrtf(*(buf_weight_VarD + idx)) *
                                       *(buf_residual_d + idx)),
              x, y, (width / w));
        }
      ...
      if (plotWeights) {
        setPixelInCvMat(
            &_debugImages.debugImageWeightP,
            getGrayCvPixel(255 * 4 * sqrtf(*(buf_weight_VarP + idx))), x, y,
            (width / w));
        setPixelInCvMat(&_debugImages.debugImageHuberWeight,
                        getGrayCvPixel(255 * *(buf_weight_Huber + idx)), x, y,
                        (width / w));
        setPixelInCvMat(
            &_debugImages.debugImageWeightedResP,
            getGrayCvPixel(128 + (128 / 5.0f) *
                                     sqrtf(*(buf_weight_VarP + idx)) *
                                     *(buf_warped_residual + idx)),
            x, y, (width / w));
      }
    }
  ...
  affineEstimation_a_lastIt =
      sqrtf((syy - sy * sy / sw) / (sxx - sx * sx / sw));
...
#if defined(ENABLE_SSE)
Sim3ResidualStruct
Sim3Tracker::calcSim3WeightsAndResidualSSE(const Sim3 &referenceToFrame) {
  ...
  const __m128 depthVarFacs = _mm_set1_ps(
      (float)
          settings.var_weight); // float depthVarFac = var_weight;	// the
                                // depth var is over-confident. this is a
                                // constant multiplier to remedy that.... HACK
  ...
  for (int i = 0; i < buf_warped_size - 3; i += 4) {
    ...
    __m128 pz2ds = _mm_rcp_ps(_mm_mul_ps(
        _mm_mul_ps(pzs, pzs), _mm_load_ps(buf_d + i))); // 1 / (z' * z' * d)
    __m128 g0s = _mm_sub_ps(_mm_mul_ps(pzs, txs),
                            _mm_mul_ps(_mm_load_ps(buf_warped_x + i), tzs));
    g0s = _mm_mul_ps(g0s, pz2ds);

    // float g1 = (ty * pz - tz * py) / (pz*pz*d);
    __m128 g1s = _mm_sub_ps(_mm_mul_ps(pzs, tys),
                            _mm_mul_ps(_mm_load_ps(buf_warped_y + i), tzs));
    ...
    __m128 drpdds = _mm_add_ps(_mm_mul_ps(g0s, _mm_load_ps(buf_warped_dx + i)),
    ...
    __m128 w_ps = _mm_rcp_ps(_mm_add_ps(
        ...
        _mm_mul_ps(
            drpdds,
            _mm_mul_ps(drpdds, _mm_mul_ps(depthVarFacs,
                                          _mm_load_ps(buf_idepthVar + i))))));

    // float w_d = 1.0f / (sv + g2*g2*s);
    __m128 w_ds = _mm_rcp_ps(_mm_add_ps(
        _mm_load_ps(buf_warped_idepthVar + i),
        _mm_mul_ps(
            g2s, _mm_mul_ps(g2s, _mm_mul_ps(depthVarFacs,
                                            _mm_load_ps(buf_idepthVar + i))))));

    // float weighted_rp = fabs(rp*sqrtf(w_p));
    __m128 weighted_rps =
        _mm_mul_ps(_mm_load_ps(buf_warped_residual + i), _mm_sqrt_ps(w_ps));
    weighted_rps = _mm_max_ps(weighted_rps, _mm_sub_ps(zeros, weighted_rps));

    // float weighted_rd = fabs(rd*sqrtf(w_d));
    __m128 weighted_rds =
        _mm_mul_ps(_mm_load_ps(buf_residual_d + i), _mm_sqrt_ps(w_ds));
    weighted_rds = _mm_max_ps(weighted_rds, _mm_sub_ps(zeros, weighted_rds));

    // depthValid = sv > 0
    __m128 depthValid = _mm_cmplt_ps(
        zeros,
        _mm_load_ps(
            buf_warped_idepthVar +
            i)); // bitmask 0xFFFFFFFF for idepth valid, 0x000000 otherwise

    // float weighted_abs_res = sv > 0 ? weighted_rd+weighted_rp : weighted_rp;
    __m128 weighted_abs_ress =
        _mm_add_ps(_mm_and_ps(weighted_rds, depthValid), weighted_rps);

    // float wh = fabs(weighted_abs_res < huber_res ? 1 : huber_res /
    // weighted_abs_res);
    __m128 whs = _mm_cmplt_ps(weighted_abs_ress,
                              huber_ress); // bitmask 0xFFFFFFFF for 1, 0x000000
                                           // for huber_res_ponly / weighted_rp
    whs = _mm_or_ps(
        _mm_and_ps(whs, ones),
        _mm_andnot_ps(whs,
                      _mm_mul_ps(huber_ress, _mm_rcp_ps(weighted_abs_ress))));

    if (i + 3 < buf_warped_size) {
      // if(sv > 0) sumRes.numTermsD++;
      numTermsD = _mm_add_ps(numTermsD, _mm_and_ps(depthValid, ones));

      // if(sv > 0) sumRes.sumResD += wh * w_d * rd*rd;
      sumResD = _mm_add_ps(
          sumResD,
          _mm_and_ps(depthValid,
                     _mm_mul_ps(whs, _mm_mul_ps(weighted_rds, weighted_rds))));

      // sumRes.sumResP += wh * w_p * rp*rp;
      sumResP = _mm_add_ps(
          sumResP, _mm_mul_ps(whs, _mm_mul_ps(weighted_rps, weighted_rps)));
    }
    ...
    _mm_store_ps(buf_weight_d + i,
                 _mm_and_ps(depthValid, _mm_mul_ps(whs, w_ds)));
  }
  sumRes.sumResP =
      SSEE(sumResP, 0) + SSEE(sumResP, 1) + SSEE(sumResP, 2) + SSEE(sumResP, 3);
  sumRes.numTermsP = (buf_warped_size >> 2) << 2;

  sumRes.sumResD =
      SSEE(sumResD, 0) + SSEE(sumResD, 1) + SSEE(sumResD, 2) + SSEE(sumResD, 3);
  sumRes.numTermsD = SSEE(numTermsD, 0) + SSEE(numTermsD, 1) +
                     SSEE(numTermsD, 2) + SSEE(numTermsD, 3);

  sumRes.mean =
      (sumRes.sumResD + sumRes.sumResP) / (sumRes.numTermsD + sumRes.numTermsP);
...
#if defined(ENABLE_NEON)
Sim3ResidualStruct
Sim3Tracker::calcSim3WeightsAndResidualNEON(const Sim3 &referenceToFrame) {
  return calcSim3WeightsAndResidual(referenceToFrame);
}
#endif

Sim3ResidualStruct
Sim3Tracker::calcSim3WeightsAndResidual(const Sim3 &referenceToFrame) {
  ...
  float sum_rd = 0, sum_rp = 0, sum_wrd = 0, sum_wrp = 0, sum_wp = 0,
        sum_wd = 0, sum_num_d = 0, sum_num_p = 0;
  ...
  for (int i = 0; i < buf_warped_size; i++) {
    ...
    float weighted_abs_res = sv > 0 ? weighted_rd + weighted_rp : weighted_rp;
    float wh = fabs(weighted_abs_res < settings.huber_d
                        ? 1
                        : settings.huber_d / weighted_abs_res);
  ...
  if (plotSim3TrackingIterationInfo) {
    printf("rd %f, rp %f, wrd %f, wrp %f, wd %f, wp %f\n ", sum_rd / sum_num_d,
           sum_rp / sum_num_p, sum_wrd / sum_num_d, sum_wrp / sum_num_p,
           sum_wd / sum_num_d, sum_wp / sum_num_p);
  }
  return sumRes;
}
...
void Sim3Tracker::calcSim3LGSSSE(LGS7 &ls7) {
  ...
  for (int i = 0; i < buf_warped_size - 3; i += 4) {
    ...
    val4 = _mm_add_ps(val3,
                      val4); // gy + px * z_sqr * gx * py + py * py * z_sqr * gy
    ...
    val4 = _mm_add_ps(val4,
                      val3); // gx + px * px * z_sqr * gx + px * py * z_sqr * gy
    // v[4] = px * px * z_sqr * gx +
    //	   px * py * z_sqr * gy +
    //	   gx;				=> SET [4]
    J65 = val4;
    ...
    if (i + 3 < buf_warped_size) {
      ls4.updateSSE(J41, J42, J43, J44, _mm_load_ps(buf_residual_d + i),
                    _mm_load_ps(buf_weight_d + i));
      ls6.updateSSE(J61, J62, J63, J64, J65, J66,
                    _mm_load_ps(buf_warped_residual + i),
                    _mm_load_ps(buf_weight_p + i));
    } else {
      for (int k = 0; i + k < buf_warped_size; k++) {
        Vector6 v6;
        v6 << SSEE(J61, k), SSEE(J62, k), SSEE(J63, k), SSEE(J64, k),
            SSEE(J65, k), SSEE(J66, k);
        Vector4 v4;
...
#if defined(ENABLE_NEON)
void Sim3Tracker::calcSim3LGSNEON(LGS7 &ls7) { calcSim3LGS(ls7); }
#endif
...
void Sim3Tracker::calcResidualAndBuffers_debugStart() {
  if (plotTrackingIterationInfo || saveAllTrackingStagesInternal) {
    ...
    fillCvMat(&_debugImages.debugImageExternalWeights,
              cv::Vec3b(other, other, 255));
    fillCvMat(&_debugImages.debugImageWeights, cv::Vec3b(other, other, 255));
    fillCvMat(&_debugImages.debugImageOldImageSource,
              cv::Vec3b(other, other, 255));
    fillCvMat(&_debugImages.debugImageOldImageWarped,
              cv::Vec3b(other, other, 255));
    fillCvMat(&_debugImages.debugImageScaleEstimation,
              cv::Vec3b(255, other, other));
    fillCvMat(&_debugImages.debugImageDepthResiduals,
              cv::Vec3b(other, other, 255));
  }
}
...
void Sim3Tracker::calcResidualAndBuffers_debugFinish(int w) {
  if (plotTrackingIterationInfo) {
    ...
    Util::displayImage("Intensities of second_frame at transformed positions",
                       _debugImages.debugImageOldImageSource);
    Util::displayImage(
        "Intensities of second_frame at pointcloud in first_frame",
        _debugImages.debugImageOldImageWarped);
    Util::displayImage("Residuals", _debugImages.debugImageResiduals);
    Util::displayImage("DepthVar Weights",
                       _debugImages.debugImageExternalWeights);
    Util::displayImage("Depth Residuals",
                       _debugImages.debugImageDepthResiduals);
  ...
  if (saveAllTrackingStagesInternal) {
    ...
    snprintf(charbuf, 500, "save/%sresidual-%d-%d.png", packagePath.c_str(), w,
             iterationNumber);
    cv::imwrite(charbuf, _debugImages.debugImageResiduals);

    snprintf(charbuf, 500, "save/%swarped-%d-%d.png", packagePath.c_str(), w,
             iterationNumber);
    cv::imwrite(charbuf, _debugImages.debugImageOldImageWarped);

    snprintf(charbuf, 500, "save/%sweights-%d-%d.png", packagePath.c_str(), w,
             iterationNumber);
